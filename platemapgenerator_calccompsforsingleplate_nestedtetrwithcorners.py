...
compsint2=[[b, c, (intervs-a-b-c), a] for a in numpy.arange(0,intervs+1, 2)[::-1] for b in numpy.arange(a,intervs+1-2*a) for c in numpy.arange(a, intervs+1-2*a-b)][::-1]
compsint2+=[[d, a, b, c] for a, b, c, d in compsint2 if not [d, a, b, c] in compsint2]
compsint2+=[[c, d, a, b] for a, b, c, d in compsint2 if not [c, d, a, b] in compsint2]
compsint2+=[[b, c, d, a] for a, b, c, d in compsint2 if not [b, c, d, a] in compsint2]
compsint2+=[cc for cc in compsint if max(cc)>cmi and not (cc in compsint2)]

comps=numpy.float32(compsint2)/intervs
comps=numpy.array([comps.T[1], comps.T[2], comps.T[3], comps.T[0]]).T
print len(comps)
